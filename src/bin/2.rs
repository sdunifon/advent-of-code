...
fn main() {
    ...
    let count = INPUT
        .iter()
        .map(|&input_string| {
            let pass_phrase = PassPhrase::new(input_string.into());
            // dbg!(&pass_phrase);
            dbg!(&pass_phrase);
            dbg!(pass_phrase.is_valid());
            pass_phrase.alternate_valid()
        })
        .filter(|x| *x)
        .count();
    println!("count:{}", count);
...
impl PassPhrase {
    fn new(str: String) -> PassPhrase {
        lazy_static! {
            static ref RE: Regex = Regex::new(r"(\d+)-(\d+)\s+(\w):\s+(\w+)").unwrap();
        }
        let mut ret: Option<PassPhrase> = None;
    ...
    fn is_valid(&self) -> bool {
        let count = self.password.matches(&self.char).count();
        dbg!(count);
        count >= self.min && count <= self.max
    }
    fn alternate_valid(&self) -> bool {
        let is_min =
            self.password.chars().nth(self.min - 1).unwrap() == self.char.chars().nth(0).unwrap();
        let is_max =
            self.password.chars().nth(self.max - 1).unwrap() == self.char.chars().nth(0).unwrap();
        is_min ^ is_max
    }
}
